...
SEC1=$(http --ignore-stdin --print=b POST :3001/api/secretaries \
  ...
  first_name="Ana" last_name="García" shift="mañana" entry_time="08:00:00" exit_time="16:00:00" email="ana.garcia@clinic.com" | jq -r .secretary.secretary_id)

SEC2=$(http --ignore-stdin --print=b POST :3001/api/secretaries \
  Authorization:"Bearer $TOKEN" \
  first_name="María" last_name="López" shift="tarde" entry_time="16:00:00" exit_time="00:00:00" email="maria.lopez@clinic.com" | jq -r .secretary.secretary_id)
...
http --ignore-stdin POST :3001/api/auth/register \
  Authorization:"Bearer $TOKEN" \
  username="ana.garcia" email="ana.garcia@clinic.com" password="Admin1234" role="secretary" entity_id:=$SEC1 nombre="Ana García"

http --ignore-stdin POST :3001/api/auth/register \
  Authorization:"Bearer $TOKEN" \
  username="maria.lopez" email="maria.lopez@clinic.com" password="Admin1234" role="secretary" entity_id:=$SEC2 nombre="María López"
...
http --ignore-stdin POST :3001/api/auth/register \
  Authorization:"Bearer $TOKEN" \
  username="roberto.hernandez" email="roberto.hernandez@clinic.com" password="Admin1234" role="doctor" entity_id:=1 nombre="Roberto Hernández"

http --ignore-stdin POST :3001/api/auth/register \
  Authorization:"Bearer $TOKEN" \
  username="laura.diaz" email="laura.diaz@clinic.com" password="Admin1234" role="doctor" entity_id:=2 nombre="Laura Díaz"

echo "✅ Usuarios de doctores creados"

# 5. CREAR HORARIOS DE CONSULTA
...
HISTORIAL=(
  "1,1,2024-01-15,Hipertensión arterial controlada,Mantener medicación actual y dieta baja en sal,Paciente responde bien al tratamiento"
  "2,1,2024-01-16,Angina de pecho,Nitroglicerina de rescate y control estricto,Requiere seguimiento cercano"
  "4,2,2024-01-17,Dermatitis atópica,Cremas hidratantes y evitar irritantes,Mejora significativa con el tratamiento"
)

for hist in "${HISTORIAL[@]}"; do
  IFS=',' read -r patient_id doctor_id date diagnosis treatment observations <<< "$hist"
  http --ignore-stdin POST :3001/api/medical-history \
    Authorization:"Bearer $TOKEN" \
    patient_id:=$patient_id date="$date" attending_doctor_id:=$doctor_id diagnosis="$diagnosis" treatment="$treatment" observations="$observations"
done
...
# 11. CREAR RELACIONES DOCTOR-OBRA SOCIAL
echo "�� Creando relaciones doctor-obra social..."
RELACIONES=(
  "1,1,true"
  "1,2,true"
  "2,1,true"
...
for rel in "${RELACIONES[@]}"; do
  IFS=',' read -r doctor_id insurance_id is_active <<< "$rel"
  http --ignore-stdin POST :3001/api/doctor-health-insurances \
    Authorization:"Bearer $TOKEN" \
    doctor_id:=$doctor_id insurance_id:=$insurance_id is_active:=$is_active
done

echo "✅ Relaciones doctor-obra social creadas"

# 12. VERIFICAR DATOS CREADOS
echo "🔍 Verificando datos creados..."
echo "📊 Resumen de datos:"
echo "Secretarias: $(http --ignore-stdin --print=b GET :3001/api/secretaries Authorization:"Bearer $TOKEN" | jq '.secretaries | length')"
echo "Citas: $(http --ignore-stdin --print=b GET :3001/api/appointments Authorization:"Bearer $TOKEN" | jq '.appointments | length')"
echo "Prescripciones: $(http --ignore-stdin --print=b GET :3001/api/prescriptions Authorization:"Bearer $TOKEN" | jq '.prescriptions | length')"
echo "Historial médico: $(http --ignore-stdin --print=b GET :3001/api/medical-history Authorization:"Bearer $TOKEN" | jq '.records | length')"
echo "Relaciones paciente-doctor: $(http --ignore-stdin --print=b GET :3001/api/patient-doctors Authorization:"Bearer $TOKEN" | jq '.relations | length')"
...
echo "   Secretaria 1: ana.garcia / Admin1234"
echo "   Secretaria 2: maria.lopez / Admin1234"
echo "   Doctor 1: roberto.hernandez / Admin1234"
echo "   Doctor 2: laura.diaz / Admin1234"