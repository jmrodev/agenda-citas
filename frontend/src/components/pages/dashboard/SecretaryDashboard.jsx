...
// import { debugDashboard, debugPatients, debugCalendar } from '../../../utils/debug.js'; // Comentado para limpieza
...
const SecretaryDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        const pacientesData = pacRes.ok ? await pacRes.json() : { totalPacientes: 0 };
        ...
        const pagosData = pagosRes.ok ? await pagosRes.json() : { totalPagos: 0 };
        ...
        setStats({ citas: 0, pacientes: 0, pagos: 0 }); // Estado por defecto en caso de error
      }
    };
  ...
  // debugDashboard('StatCards configuradas:', statCardsData);
  ...
  return (
    <DashboardLayout title='Dashboard de Secretaría'>
      ...
      <QuickActionsBar actions={secretaryActions} />

      {/* Aquí podrían ir otros componentes específicos del dashboard de secretaría,
          como lista de pacientes que esperan confirmación, tareas pendientes, etc. */}
    </DashboardLayout>
  );
};
